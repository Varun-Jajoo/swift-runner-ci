...
//    }
//    
//    func genericAPICall(serviceEndPoint: ApiEndPoint, method: HTTPMethod, queries: [String: String]? = nil, parameters: Parameters? = nil, interceptor: RequestInterceptor? = nil, isShowLoading: Bool = false, isShowLoadingWithoutMsg: Bool = false, completion: @escaping ((Data?, Error?) -> Void)){
//        
//        let url = serviceEndPoint.getURL(queries: queries)
...
//            return
//        }
//        // Show Loader without Msg
//        if isShowLoadingWithoutMsg {
//            Utility.showLoader(title: "", subtitle: "")
//        }
//        
//        // Show Loader
//        if isShowLoading{
...
//                        AlertHelper.shared.showCustomeAlert(message: error.localizedDescription)
    ...
    func genericAPICall(serviceEndPoint: ApiEndPoint, method: HTTPMethod, queries: [String: String]? = nil, parameters: Parameters? = nil, interceptor: RequestInterceptor? = nil, isShowLoading: Bool = false, isShowLoadingWithoutMsg: Bool = false, completion: @escaping ((Data?, Error?) -> Void)){
        ...
        APILogger.logRequest(urlRequest, parameters: parameters)
...
final class APILogger {
    ...
    static func logRequest(_ request: URLRequest, parameters: Any?) {
...
//        if let token = request.value(forHTTPHeaderField: "Authorization") {
        print("🔐 Token: \(appUserDefaults.getAccessToken() ?? "🔐 Token: ❌ Not Found")")
//        } else {
    ...
    static func logResponse(data: Data?, response: HTTPURLResponse?, error: Error?) {
        ...
        if let headers = response?.allHeaderFields,
           let token = headers["Authorization"] {
            print("🆕 New Token From Server: \(token)")
        }
