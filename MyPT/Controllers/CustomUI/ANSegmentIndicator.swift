//
//  ANSegmentIndicatorSettings.swift
//  MyPT
//
...
class ANSegmentIndicator: UIView {
    ...
    var settings: ANSegmentIndicatorSettings = ANSegmentIndicatorSettings() {
        didSet {
            self.layer.sublayers?.removeAll(where: { $0.name == "ANSegmentIndicator_layer" })
            self.setup()
        }
    }
    ...
    private func drawDynamicSegments() {
        ...
        let summedSpaceForSegments = 2 * .pi - emptySpaceInRadians
        let spaceCorrelation = toRadians(settings.segmentWidth)
        ...
        let startPointPaddingInRadians = toRadians(settings.startPointPadding - 90) // Start from top
    ...
    private func drawStaticSegments() {
        staticSegments = []
        guard settings.isStaticSegmentsVisible else { return }
    ...
    private func getSegment(startAngle: Radians,
        ...
        let radius = bounds.height / 2 - settings.segmentWidth / 2
        let bezierPath = UIBezierPath(arcCenter: centre,
                                      radius: radius,
        ...
        segmentLayer.name = "ANSegmentIndicator_layer"
    ...
    private func drawPassedSegments(activeSegment: Int) {
        guard activeSegment > 0 else { return }
    ...
    func updateProgress(percent: Degrees) {
        ...
        if self.activeSegment != activeSegment {
            drawSegmentToFull(index: self.activeSegment) {
                self.value = 0.0
