...
protocol RulerRangeScrollViewDelegate: AnyObject {
    func rulerScrollViewDidReachThreshold(_ scrollView: RulerRangeScrollView, currentValue: Float)
}

class RulerRangeScrollView: UIControl, UIScrollViewDelegate {

    open var sideOffset: CGFloat = kDefaultScrollViewSideOffset
    open var direction: RulerLayerDirection = .horizontal
    open var colorOverrides: Dictionary<RulerRange<Float>, UIColor>?
    open var range: RulerRange<Float> = RulerRange<Float>(location: 0, length: 0) {
        didSet {
            setupScrollView()
            currentValue = ceilf((range.location + range.length) / 2.0)
        }
    }
    var markerTypes: Array<RulerRangeMarkerType>? {
        didSet {
            setupScrollView()
        }
    }

    weak var delegate: RulerRangeScrollViewDelegate?
    private var lastThresholdTriggered: Float = 0
    private var thresholdStep: Float = 80
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if !automaticallyUpdatingScroll {
            ...
            if oldValue != currentValue {
                ...
                if currentValue >= lastThresholdTriggered + thresholdStep {
//                    lastThresholdTriggered = currentValue
                    delegate?.rulerScrollViewDidReachThreshold(self, currentValue: currentValue)
                  
                    if thresholdStep <= 50000 {
                        thresholdStep  += 100
                    }
                }
            }
    ...
    func valueForContentOffset(contentOffset: CGPoint) -> Float {
        ...
        switch self.direction {
        case .horizontal:
            return Float(rangeStart) + Float(contentOffset.x + self.scrollView.contentInset.left) / Float(self.offsetCoefficient())
        case .vertical:
            return Float(rangeStart) + Float(self.rangeLayer.frame.height -
    ...
    func offsetCoefficient() -> CGFloat {
        switch self.direction {
        case .horizontal:
            return self.rangeLayer.frame.width / CGFloat(abs(range.length))
        case .vertical:
            return self.rangeLayer.frame.height / CGFloat(abs(range.length))
        }
    }
    ...
    override func layoutSubviews() {
        ...
        case .horizontal:
            ...
            self.scrollView.contentInset = UIEdgeInsets(top: 0, left: sideInset - self.sideOffset, bottom: 0, right: sideInset - self.sideOffset)
        case .vertical:
            let sideInset = self.scrollView.frame.height / 2.0
            self.scrollView.contentInset = UIEdgeInsets(top: sideInset - self.sideOffset, left: 0, bottom: sideInset - self.sideOffset, right: 0)
        }
...
class RulerRangeScrollView: UIControl, UIScrollViewDelegate {
    ...
    */
    // self.rangeLayer -> It is used for show rular
    override func layoutSubviews() {
        super.layoutSubviews()
//        self.rangeLayer.backgroundColor = UIColor.clear.cgColor //only for testing
