...
class RulerRangeMarker: NSObject {
    ...
    var value: Float = 0.0
    var alignment: RulerMarkerVerticalAlignment = RulerMarkerVerticalAlignment.bottom
    var text: String = ""
    var textAlignment: RulerMarkerVerticalAlignment = RulerMarkerVerticalAlignment.bottom
...
class RulerRangeLayer: CALayer {
    var direction: RulerLayerDirection = .horizontal
    var markerTypes: Array<RulerRangeMarkerType> = Array()
    var colorOverrides: Dictionary<RulerRange<Float>, UIColor>?
    ...
    lazy var markers: Array<RulerRangeMarker> = self.initializeMarkers()
    ...
    func initializeMarkers() -> Array<RulerRangeMarker> {
    ...
    */
    ...
    /*
        ...
        marker
    */
    func drawMarkerInHorizontalDirection(_ marker: RulerRangeMarker, at pos: CGFloat, in context: CGContext) {
        let rangeEnd = fmax(self.range.location, self.range.location + self.range.length)
        ...
        case .bottom:
            textYPos = textSize.height + marker.type.size.height + 20
        case .center:
...
//        let markerTextRect = CGRect(x: textXPos, y: -textYPos, width: textSize.width, height: textSize.height)
    ...
    func drawMarkerInVerticalDirection(_ marker: RulerRangeMarker, at pos: CGFloat, in context: CGContext) {
        let rangeEnd = fmax(self.range.location, self.range.location + self.range.length)
        let colorOverride = self.colorOverride(for: marker.value)
        let color = colorOverride ?? marker.type.color
        let textAttributes = [
