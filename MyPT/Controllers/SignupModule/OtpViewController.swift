    ...
    @IBOutlet weak var viewBackground: UIView!
        ...
        @objc func getTime() {
            ...
            if counter > 0 {
                ...
                setResendLabel(counter: counter)
                resentOtpLbl.isUserInteractionEnabled = false
            } else {
                stopTimer()
                timerFinished()
        ...
        func timerFinished() {
            ...
            resentOtpLbl.font = AppFont.semibold.size(12.0, familyName: familyFunnelSans)
            resentOtpLbl.textColor = UIColor.appWhite
            resentOtpLbl.isUserInteractionEnabled = true
        }
        ...
        func setResendLabel(counter: Int?) {
            
            let normalText = "Resend OTP in  "
            let counterText = counter != nil ? "\(counter!) seconds" : ""
            
            let fullText = normalText + counterText
            
            let attributed = NSMutableAttributedString(string: fullText)
            
            // 🔸 Font for "Resend OTP in"
            attributed.addAttributes([
                .font: AppFont.regular.size(12.0, familyName: familyManrope),
                .foregroundColor: UIColor.txtDarkGray
            ], range: (fullText as NSString).range(of: normalText))
            ...
            if let counter = counter {
                ...
                ], range: (fullText as NSString).range(of: "\(counter) seconds"))
            }
            
            resentOtpLbl.attributedText = attributed
        }
