...
//            cell.addLeftBorder(borderColor: UIColor.appYellow)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // Shared with Home's "Upcoming Bookings" tap handler - see
        // GroupClassBookingRouter's own doc comment for why this must not be
        // duplicated inline again.
        if let row = self.bookingData?[indexPath.row], row.isGroupClass {
            GroupClassBookingRouter.route(row: row, isUpcomingTab: self.selectedTags == 2, on: self.navigationController)
            return
        }
